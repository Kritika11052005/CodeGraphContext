...
class JavaToolkit:
    ...
    def __init__(self, code_finder: Any = None):
        """
        Args:
            code_finder: A ``CodeFinder`` instance (or compatible duck-type).
        """
        self._cf = code_finder
        self._driver = code_finder.driver if code_finder is not None else None
    ...
    def get_cypher_query(self, query: str) -> str:
        ...
        raise ValueError(f"Unsupported Java query type: {query}")
